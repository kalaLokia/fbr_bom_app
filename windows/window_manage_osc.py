...
class WindowManageOsCharges(QtWidgets.QWidget):
    ...
    def refreshDataModel(self):
        ...
        for row, osc in enumerate(self.os_charges):
            ...
            print = QtGui.QStandardItem(str(osc.printing))
            stitch = QtGui.QStandardItem(str(osc.stitching))
            print.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            stitch.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.model.setItem(row, 0, article)
            self.model.setItem(row, 1, print)
            self.model.setItem(row, 2, stitch)
