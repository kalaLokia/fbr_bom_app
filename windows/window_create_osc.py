...
from core.threads.thread_create_os_charges import WorkerThreadOsCharges
from ui.ui_create_osc import Ui_DialogUpdateFromFile


class WindowCreateOsCharges(QtWidgets.QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.ui.btn_choose_file.clicked.connect(self.chooseOsChargesPath)
    ...
    def updateTable(self):
        ...
        os_charges_path = self.ui.text_file_path.text()

        if not os.path.exists(os_charges_path):
            self.event_show_message(False, "Invalid path to file given for OS Charges.")
            return
        ...
        self.worker = WorkerThreadOsCharges(os_charges_path)
        self.worker.start()
    ...
    def event_confirm_action(self):
        ...
        dialog.setWindowIcon(QtGui.QIcon("icons/gear-solid.svg"))
        dialog.setText(
            "You are going to delete all current outsource charges data from the database, which cannot be restored later.\n\nDo you want to proceed the operation anyway?"
        )
